    ...
    <>
      ...
      {tab === 'cobertura' ? (
        <Card>
          ...
          <div className="mt-5 app-table-scroll rounded-xl border border-[var(--app-border)] overflow-hidden"><table className="app-table text-sm"><thead><tr><th scope="col" className="p-3">Viagem</th><th scope="col" className="p-3">Clientes</th><th scope="col" className="p-3">CE / Taxas</th><th scope="col" className="p-3">NOA</th><th scope="col" className="p-3">NOR</th><th scope="col" className="p-3">NOB</th></tr></thead><tbody>{(coverageQuery.data ?? []).map((row) => <tr key={row.voyageId}><td className="p-3 font-medium">{row.vesselName} · {row.voyageNumber}</td><td className="p-3">{row.customers}</td><td className="p-3"><Badge tone={row.finance.pending ? 'yellow' : 'green'}>{row.finance.sent}/{row.finance.ready} enviados · {row.finance.pending} pendentes</Badge></td><td className="p-3"><Badge tone={row.noa.sent >= row.noa.total ? 'green' : 'yellow'}>{row.noa.sent}/{row.noa.total}</Badge></td><td className="p-3"><Badge tone={row.nor.sent >= row.nor.total ? 'green' : 'yellow'}>{row.nor.sent}/{row.nor.total}</Badge></td><td className="p-3"><Badge tone={row.nob.sent >= row.nob.total ? 'green' : 'yellow'}>{row.nob.sent}/{row.nob.total}</Badge></td></tr>)}{!coverageQuery.data?.length ? <tr><td colSpan={6} className="p-8 text-center text-[var(--app-muted)]">Nenhuma viagem encontrada.</td></tr> : null}</tbody></table></div>
        </Card>
      ) : tab === 'disparo' ? (
