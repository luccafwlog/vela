...
export async function applyChangeOfDestination(input: ApplyCodInput): Promise<ApplyCodResult> {
  ...
  if (!Number.isInteger(omissionId) || omissionId <= 0) {
    throw new Error('Alteração de destino exige uma omissão vinculada; use o fluxo COD auditado.')
  }
  ...
  // Toda alteração de POD passa pela RPC COD, que aplica a disposição,
  // reconcilia financeiro e grava a auditoria na mesma transação.
