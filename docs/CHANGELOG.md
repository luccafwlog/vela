...
- **Remediação de segurança da auditoria do Portal e superfície F12 (2026-09-22):**
  Logout do Portal passa a falhar fechado com timeout de segurança (5s) e purga local forçada de tokens (`removeLocalSessionFallback`), exibindo alerta contextual no login caso a revogação remota falhe por instabilidade de rede (`isSigningOut`). Uploads de anexos em disputas de demurrage têm escrita direta no Storage revogada para clientes do Portal (`demurrage_dispute_objects_insert` restrita a `is_active_user()`), passando a ser orquestrados pela nova Edge Function `portal-dispute-attachment` com validação de assinatura/magic bytes (PDF, PNG, JPEG, TXT) e tamanho (máx 10 MB), cota agregada no banco restrita a disputas abertas com lock transacional consultivo (`pg_advisory_xact_lock`), limite diário de uploads, delegação autenticada para a RPC `add_demurrage_dispute_attachment`, limpeza imediata de arquivos órfãos em caso de falha de metadados e rotina administrativa `cleanup_orphaned_dispute_attachments` (migration `074`). Recuperação de senha desacopla processamento para segundo plano via `EdgeRuntime.waitUntil`, retornando 202 imediatamente após checagem de rate limit e eliminando canal lateral temporal. O build de produção remove recursivamente e falha fechado na detecção de artefatos proibidos (`.map` ou `.vite`), e dependências foram atualizadas (0 vulnerabilidades em `npm audit`).
  [Plano em execução](plans/2026-09-22-plano-remediacao-auditoria-portal-f12.md) e PR #718.
