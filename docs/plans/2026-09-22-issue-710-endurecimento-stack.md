...
- Evidência complementar de 2026-09-22: os jobs da PR #734 para `efb54432` não chegaram a iniciar runner/etapas. A anotação do GitHub Actions dizia haver pagamento recente pendente ou limite de gastos que precisaria ser aumentado; isso não era falha comprovada do código. O owner informou que resolveu a pendência; o próximo commit será usado para confirmar que os runners voltaram. Não foi solicitado nem executado aumento de limite ou upgrade de plano.
- M7 continua sem prova de runtime: a chamada OPTIONS não autenticada ao endpoint do Preview foi barrada com HTTP 403 e erro Cloudflare 1000 antes de aparecer nos logs de API Gateway; portanto não comprova se `CF-Connecting-IP` é sobrescrito. Não promover o rate limit Redis sem essa prova.
- M9: o painel EU do PostHog confirmou que ainda não há eventos. A PR #735 tem testes/CI e previews Vercel verdes, mas não existe conta/invoice fixture acessível no Portal Preview desta sessão; não foi fabricado evento diretamente no PostHog. A ingestão real segue pendente.
- M10: na branch atual da PR #734, `backup:r2:test` passou (16/16) e o dry-run não acessou banco, arquivo ou R2. A tentativa da AWS CLI ainda não foi concluída: o log do Windows Installer registra o início do MSI às 22:49 local, o serviço continua ativo e `winget list` não encontra o pacote. Não iniciar outro instalador nem agendar/executar backup real até resolver essa transação e guardar a chave de cifragem na cópia externa já escolhida.
