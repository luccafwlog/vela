...
- No commit `c21c8b7c` da PR #734, CI, testes, replay Postgres e Supabase Preview passaram; os dois checks Vercel falharam por limite de builds do plano Hobby (“retry in 24 hours”), sem upgrade contratado. Não há merge e permanece `REVIEW_REQUIRED`.
- M3: a PR #736 implementa métricas agregadas para início/fim, contagem explícita, duração e resultado dos quatro runners aprovados, sem IDs/payload; runtime Sentry/Edge, `SENTRY_DSN`, ingestão e validação de quota permanecem pendentes. Os checks verdes no head anterior não provam ingestão.
...
- [x] Implementar no código métricas Sentry agregadas de início/fim dos quatro runners autorizados, duração, contagem com `count_kind` explícito e resultado de baixa cardinalidade, sem IDs nem payload. O wrapper observa somente invocações autenticadas. A implementação usa a API Application Metrics do SDK 10.73.0; runtime/ingestão, DSN Edge e cota/consumo ainda precisam ser validados no Preview e no painel antes de produção, mantendo o limite de custo zero.
