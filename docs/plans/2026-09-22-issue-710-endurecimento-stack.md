...
- M10: na branch da PR #734, `backup:r2:test` passou (16/16) e o dry-run não acessou banco, arquivo ou R2. A tentativa da AWS CLI ainda não foi concluída: o log do Windows Installer registra o início do MSI às 22:49 local, o serviço continua ativo e `winget list` não encontra o pacote. Não iniciar outro instalador nem agendar/executar backup real até resolver essa transação e guardar a chave de cifragem na cópia externa já escolhida.
