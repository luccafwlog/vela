...
- **PR Cloudflare Pages #739 (2026-09-23):** aberta como draft após autorização do owner. CI GitHub (build/bundle, docs/lint, migration replay, security guard e os três shards de testes) passou; os dois previews Vercel passaram; `Supabase Preview` foi cancelado pela integração ao atingir o limite de branches concorrentes. A publicação Cloudflare não executou: os workflows `workflow_run` só são elegíveis depois que estiverem no branch padrão e o publish exige a variável-gate ainda ausente. Nenhum deployment Pages, mudança de Access/DNS ou tráfego de produção foi criado. O owner autorizou abrir esta PR aceitando possível rate limit Vercel; essa autorização não abrange merge, primeiro deployment Pages, mudança de `CLOUDFLARE_PAGES_ACCESS_CONFIGURED`, DNS ou deploy de produção. Resolver a capacidade de Preview Supabase e obter decisão própria antes de merge/publicação.
...
- **Automação Pages em PR (2026-09-23):** código está no branch `codex/cloudflare-pages-migration` e PR draft #739, ainda não em `main`; inclui staging das duas SPAs, workflow `workflow_run` que aguarda Supabase Preview, e job de publicação isolado do código da PR. O publish exige `vars.CLOUDFLARE_PAGES_ACCESS_CONFIGURED == 'true'`; a variável ainda não existe e só poderá ser criada após o teste de Access em ambos projetos. O token Cloudflare fica apenas nos jobs confiáveis de publicação/limpeza. Validações locais: 15 testes específicos, lint, typecheck, build, build+staging Pages, suíte serial completa (649 arquivos aprovados, 28 ignorados; 3.512 testes aprovados, 135 ignorados), `docs:check` e `git diff --check` passaram. CI GitHub da #739 passou; Supabase Preview foi cancelado por limite de branches concorrentes; os dois checks Vercel passaram. Nenhuma execução Pages, deployment, domínio ou DNS ocorreu.
