...
> **Estado:** execução autorizada iniciada em 2026-09-23; Zero Trust Free ativo; os projetos Cloudflare Pages `vela-internal` e `vela-portal` foram criados vazios; ambos têm Access obrigatório nas URLs Preview e a política só permite `luccafwlog@gmail.com`. O owner escolheu e configurou manualmente o IdP Cloudflare nos dois apps (conta acessada via Google); MFA permanece desativado conforme decisão anterior. O painel autenticado confirma Cloudflare como único provedor, sessão de 24 h e regra herdada Include Emails para o Gmail da conta. A URL de Preview sem deployment redirecionou ao consentimento OAuth Cloudflare; o owner informa agora que completou a autenticação nos dois apps. Ainda não há deployment para provar fluxo autorizado e negado. Fase 0.5 segue em implementação local. Em 2026-09-23, o owner escolheu migrar a hospedagem estática do Vela e do Portal para Cloudflare Pages antes do GO-LIVE, mantendo Supabase como backend e Vercel temporária durante validação. DNS/cutover e desligamento final ainda dependem de evidência e confirmação operacional.
>
...
- **Gate imediato de hospedagem:** os termos atuais da Vercel dizem que Hobby é apenas para uso pessoal/não comercial, e a política de uso justo diz que uso comercial exige Pro/Enterprise. A conta observada está em Hobby. O preço atual do Pro começa em US$ 20/mês, com uma vaga de deploy incluída e US$ 20 de crédito de uso; cobrança adicional pode ocorrer por consumo excedente. [Termos](https://vercel.com/legal/terms) · [uso justo](https://vercel.com/docs/limits/fair-use-guidelines) · [Pro](https://vercel.com/docs/plans/pro-plan). Enquanto o owner não escolher um caminho compatível com a preferência de custo, não abrir/atualizar PR que dispare novo Preview nem promover Production.
