...
- **Decisão operacional (2026-09-23):** o owner autorizou abrir agora a PR da migração Pages, aceitando que o check Vercel possa falhar por rate limit. Isso autoriza publicar o branch e abrir a PR; não autoriza merge, deployment Pages, mudança de `CLOUDFLARE_PAGES_ACCESS_CONFIGURED`, DNS ou deploy de produção. Os workflows Pages de `workflow_run` só passam a existir no branch padrão depois do merge; nenhum deploy Cloudflare será disparado só por esta abertura de PR. A primeira publicação controlada e o teste Access exigem uma decisão operacional separada quando a PR estiver revisada e os checks conhecidos.
