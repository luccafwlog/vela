...
- A RPC `import_baplie_staging_transactional` exige Administrativo no banco. A tela `/baplie` só mostra importar/reimportar ao Administrativo, mas a ação rápida da aba Importação da Viagem mostra o botão a todos, e o não-admin recebe `42501`. A decisão de liberar a todo Departamento está no [plano de alinhamento](../plans/2026-09-23-alinhamento-apresentacao-docs-codigo.md).
