...
`src/pages/PortalLogin.tsx` aceita CNPJ, senha e token efêmero do Turnstile. Se a sessão já foi hidratada, redireciona para `/portal`; durante submit chama `usePortalAuth.signIn`, que invoca `portal-login`; a Edge Function valida o token antes do rate limit, da consulta de conta e do Supabase Auth. Falhas de credencial continuam genéricas; recusa do CAPTCHA permite refazer a verificação. Antes de chamar o servidor, a tela reprova CNPJ com menos de 14 caracteres (`isCompleteCnpjLogin`, `src/lib/portalCnpjLogin.ts`) com mensagem própria de digitação — o formato se confere offline, então avisar não revela nada sobre a base, e a tentativa não consome o rate limit de login.

### `/portal/esqueci-senha`

`src/pages/PortalForgotPassword.tsx` envia o CNPJ e o token efêmero de ação
`portal_recovery` para a Edge Function `portal-password-recovery`. A Edge valida
o token antes de consultar/registrar rate limit ou disparar trabalho de email.
A resposta é `{ accepted: true }` para todo caso
...
true})`, mantido em estado para o submit) e envia `{ token, password,
turnstile_token }` à Edge Function `portal-password-reset`. A função valida o
Turnstile antes de consultar/consumir o convite e trocar a senha via Auth Admin;
depois revoga as sessões do Portal.

### `/portal/ativar`

O link público mantém o token do convite somente em estado da página. Antes de
consultar se o convite é válido ou revelar nome/CNPJ mascarado, a página exige
Turnstile com action `portal_activation`. O token de desafio é de uso único;
após inspeção bem-sucedida a tela exige uma nova verificação para concluir a
ativação e envia o token ao servidor junto com a senha. A Edge Function valida
Siteverify antes de ler/consumir o convite ou chamar Auth Admin.
...
| Tela / ação | Pré-condições | Origem | Orquestração | Persistência | Efeitos e cache | Falhas | Evidência |
|---|---|---|---|---|---|---|---|
| `/portal/login` — autenticar por CNPJ | CNPJ canônico de 14 posições e senha preenchidos; token Turnstile de ação `portal_login`; pontuação colada é removida imediatamente e letras são preservadas em maiúsculas; comprimento < 14 para na tela (`isCompleteCnpjLogin`) | `PortalLogin` → widget Turnstile → `usePortalAuth.signIn` | Edge Function `portal-login` valida Siteverify/action/hostname antes do rate limit, consulta ou Auth técnico → `setSession` | `customer_portal_accounts.login_cnpj`; rate limit hash em `portal_login_attempts` | O navegador recebe somente tokens de sessão; token Turnstile é de uso único | CNPJ desconhecido, conta inativa, senha errada e bloqueio usam a mesma mensagem; falha Turnstile é recuperável; no caminho bloqueado a consulta à conta e o alerta `portal_abuso_login` rodam em `EdgeRuntime.waitUntil`, então o 401 sai sem trabalho a mais; CNPJ incompleto tem mensagem de digitação, decidida sem consultar o servidor | **Teste:** `src/hooks/__tests__/usePortalAuth.test.tsx`, `src/pages/__tests__/PortalLogin.test.tsx`, `src/services/__tests__/turnstile.test.ts`, `src/services/__tests__/portalTurnstileIntegration.test.ts` |
| Inicialização — hidratar sessão e overview | Aplicação montada; sessão Portal persistida opcional | `PortalAuthProvider` | `getSession()` → `fetchOverview()`; listener `onAuthStateChange` reidrata em `SIGNED_IN`/`TOKEN_REFRESHED` quando necessário | Supabase Auth; RPC `portal_get_session_overview_v2`; UPDATE de `last_login_at` | Define `overview`, `isAuthenticated=Boolean(overview)`; Sentry limpa a identidade com `setUser(null)` e mantém apenas tag `area=portal` | Sessão sem Conta de Portal ativa gera `28000` e limpa overview; outras falhas não são exibidas | **Teste:** `src/hooks/__tests__/usePortalAuth.test.tsx` |
...
| `/portal/ativar` — consultar convite e criar senha | Token de convite presente; Turnstile `portal_activation` validado separadamente na inspeção e na ativação | `PortalAtivacao` → widget Turnstile | Edge Function valida Siteverify antes de ler/consumir `portal_invites` ou chamar Auth Admin | `portal_invites`; `customer_portal_accounts`; Supabase Auth Admin | O desafio é renovado entre inspeção e ativação; o token do convite é removido da URL e mantido apenas em estado | Link inválido/expirado permanece genérico; falha Turnstile permite nova tentativa | **Teste:** `src/pages/__tests__/PortalAtivacao.test.tsx`, `src/services/__tests__/portalTurnstileIntegration.test.ts` |
| `/portal/esqueci-senha` — solicitar recuperação | CNPJ de 14 posições numéricas ou alfanuméricas; pontuação colada é removida imediatamente; token Turnstile `portal_recovery`; comprimento < 14 para na tela (`isCompleteCnpjLogin`) | `PortalForgotPassword.handleSubmit` → widget Turnstile | Edge Function valida Siteverify antes do rate limit em `portal_recovery_check_rate_limit`/`_register_failure`; trabalho de busca, supressão, convite e envio roda em segundo plano via `EdgeRuntime.waitUntil` | `customer_portal_accounts.login_cnpj`; `portal_invites` (purpose `recuperacao`); email via `sendPortalEmail` | `{ accepted: true }` sai imediatamente após rate limit, eliminando canal lateral temporal (TTFB uniforme); `{ accepted: false, rate_limited: true }` no rate limit | Resposta não enumera conta (achado 3.2 e PAF-04); confirmação afirma o envio sem condicionar a existência de conta; falha Turnstile é recuperável; rate limit mostra "tente mais tarde"; teto de envio de um email por hora por conta (validade do convite) | **Código:** `src/pages/PortalForgotPassword.tsx`, `supabase/functions/portal-password-recovery/index.ts`, [ADR 0049](../adr/0049-rate-limit-do-portal-chaveado-somente-por-cnpj.md); **Teste:** `src/pages/__tests__/PortalRecovery.behavior.test.tsx`, `src/services/__tests__/portalEdgeFunctionsOrder.test.ts`, `src/services/__tests__/turnstile.test.ts` |
...
| `/portal/recuperar-senha` — atualizar senha | Token de convite de recuperação válido na query string; senha 8+; confirmação igual; token Turnstile `portal_password_reset` | `PortalResetPassword.handleSubmit` → widget Turnstile | Edge Function valida Siteverify antes do hash/consumo do convite e chama Auth Admin `updateUserById` | `portal_invites` (consumo condicional); `revokePortalSessions` | Remove `token` da URL após leitura (achado 3.3); encerra sessões do Portal e navega para `/portal/login` | Token ausente/expirado/inválido mostra mensagem única; falha Turnstile recuperável; validação local de senha | **Teste:** `src/pages/__tests__/PortalRecovery.behavior.test.tsx`, `src/services/__tests__/turnstile.test.ts` |
...
| `/portal/billing` — abrir disputa de demurrage | Invoice não paga/cancelada e sem disputa na UI; motivo não vazio; rate limit | `DisputeModal` → `usePortalOpenDispute` | `portalOpenDemurrageDispute` | RPC `portal_open_demurrage_dispute`; UPDATE `demurrage_invoices`; INSERT `portal_notifications` e `alerts`; após sucesso, evento agregado `dispute_opened` | Invalida `['portal-demurrage-invoices']`; notificação aparece no polling/refetch próprio | 3 tentativas/30 min, motivo vazio, invoice alheia/inexistente ou disputa já aberta; erro não gera evento | **Código:** `src/components/portal/DisputeModal.tsx`, `src/hooks/usePortalDisputes.ts`; **Teste:** `src/services/__tests__/portalBillingMutations.test.ts`; **Runtime não executado** |
