...
- **Cálculo sem cliente vinculado (migration `072`):** `sync_local_charge_receivable` não aborta com erro quando `v_bl.customer_id IS NULL`. Retorna `NULL`, permitindo o cálculo e armazenamento de linhas e subtotais tarifários na importação de novos B/Ls antes da conciliação do cliente. O recebível no ledger (`bl_receivables`) é gerado assim que o cliente é conciliado.
