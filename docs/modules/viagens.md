...
- `src/components/voyages/VoyageCard.tsx` possui cinco abas locais, não sincronizadas na URL: `visao`, `importacao`, `exportacao`, `manifestos` e `adr`. As abas usam o componente `.app-tab` compartilhado.
...
- **Agency Departure Report (ADR):** a aba terminalizada organiza Escala, Carga descarregada, Vazios descarregados e Veículos em Operações/Documentação, e Granito/Embarque de vazios em Equipamentos, com resolução e sign-off por departamento. Carga descarregada conta somente os cheios dos B/Ls, com natureza (`carga_geral`/`IMO`/`OOG`) e destino (final ou transbordo) como eixos independentes — o split de destino viaja no snapshot em `sections.cargaDescarregada.destino` e é impresso como `Destino dos containers descarregados`; o Baplie aparece em Vazios descarregados como contagem física para comparação, enquanto a classificação vem do módulo de Vazios de Importação. `resolvedReportId`, o recorte por frente que pode esvaziar sem esconder a seção e a atribuição por frente inteira permanecem invariantes.
...
- Navegação contextual:
  ...
  - `/embarquevazios?voyage=<id>` (Embarque aberto com a viagem selecionada e travada);
  - `/baplie?voyage=<id>`.
...
| Navegar para BLs, Granito, vazios e Baplie | Card habilitado quando há dados; Baplie aparece para divergências | `NavigationCard` / botão “Resolver divergências” | `navigate` acrescenta `?voyage=<id>` | Nenhuma | A tela destino decide se consome o parâmetro | O Embarque de Vazios trava a viagem quando recebe `?voyage=<id>` | `src/components/voyages/VoyageCard.tsx`; `src/AppInterno.tsx`; `src/pages/Bls.tsx`; `src/pages/EmbarqueVazios.tsx` |
