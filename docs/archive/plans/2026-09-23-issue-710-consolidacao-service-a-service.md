...
- **Acesso Cloudflare (snapshot 2026-09-23; gate superado pelo bootstrap descrito abaixo):** por decisão do owner, `Cloudflare` substituiu One-time PIN em ambos os Access apps. Leitura autenticada confirma os destinos `*.vela-internal.pages.dev` e `*.vela-portal.pages.dev`, política herdada `Allow Members - Cloudflare Pages`, include somente `luccafwlog@gmail.com`, MFA da organização desativado e sessão de 24 h. O preview vazio inicialmente levou ao consentimento OAuth; o owner confirmou que concluiu autenticação em ambos os apps. A configuração e o consentimento estão concluídos; runtime autorizado/negado ainda não foi provado. A nota original exigia manter o gate ausente até o teste; essa ordem foi substituída pela conferência manual de Access antes do bootstrap e pelo teste runtime depois.
...
- **PR Cloudflare Pages #739 (snapshot na abertura, 2026-09-23):** aberta como draft após autorização do owner. CI GitHub (build/bundle, docs/lint, migration replay, security guard e os três shards de testes) passou; os dois previews Vercel passaram; `Supabase Preview` foi cancelado pela integração ao atingir o limite de branches concorrentes. A publicação Cloudflare não executou: os workflows `workflow_run` só são elegíveis depois que estiverem no branch padrão e o publish exige a variável-gate ainda ausente. Nenhum deployment Pages, mudança de Access/DNS ou tráfego de produção foi criado. Naquele momento, a autorização para abrir a PR não abrangia merge nem deploy; o owner posteriormente autorizou a sequência bootstrap descrita abaixo, ainda condicionada à aprovação formal, sem DNS/produção.
- **Retentativa da Preview Supabase #739 (2026-09-23):** após o primeiro check `skipping`, um commit vazio foi enviado com autorização explícita do owner; os demais checks CI passaram e os previews Vercel foram ignorados porque o commit não alterou os apps, mas o Supabase continuou `skipping`. O owner então confirmou ter excluído todas as Previews. Leitura autenticada da API Supabase confirmou que só resta `main`; a lista anterior mostrava Previews das PRs #719, #734 e #735, todas ainda abertas. O check da #739 segue `skipping` no SHA `1468022a90b480562da913d970d5a57e9fddb650`; portanto espaço disponível está confirmado, mas nova tentativa ainda precisa ser gerada. Não reabrir ou alterar outros PRs para retentar. O owner já autorizou uma nova tentativa; usar uma atualização real e pequena deste plano, depois verificar se o Supabase cria a Preview. A exclusão dos ambientes de Preview ligados às PRs abertas é um impacto operacional a registrar; não recriá-los sem necessidade/decisão.
- **Preview Supabase #739 criado e associado (2026-09-23):** após autorização explícita do owner para aceitar o custo temporário, `supabase_get_cost` confirmou US$ 0,01344/hora para uma branch. O ambiente foi criado sem dados de produção (`with_data=false`), saudável, id `3c2eefaa-ae1d-40b9-9f66-e8df165bfd47`, project ref `ptxopukvnzlbrwoboimv`; `supabase branches update --git-branch codex/cloudflare-pages-migration` confirmou a associação ao GitHub. A integração não retroage o check `skipping` do SHA `9db8b9d8`; uma nova sincronização é necessária. O owner autorizou remover esta branch temporária após concluir a validação. O ambiente segue ativo e faturável até a remoção; manter o tempo ativo mínimo. Ainda não há merge, deploy Pages, alteração DNS ou produção.
...
- **Automação Pages em PR (snapshot 2026-09-23; gate superado pelo bootstrap descrito abaixo):** código está no branch `codex/cloudflare-pages-migration` e PR #739, ainda não em `main`; inclui staging das duas SPAs, workflow `workflow_run` que aguarda Supabase Preview, e job de publicação isolado do código da PR. A nota original dizia que `CLOUDFLARE_PAGES_ACCESS_CONFIGURED` só poderia ser criada após runtime test; essa ordem foi substituída pela conferência das políticas no painel antes do bootstrap e teste runtime depois. O token Cloudflare fica apenas nos jobs confiáveis de publicação/limpeza. Validações locais e CI dos SHAs anteriores constam nos registros históricos abaixo; nenhuma execução Pages, deployment, domínio ou DNS ocorreu.
...
- **Snapshot de ações externas anterior à criação dos projetos Pages (2026-09-23):** na ocasião, foram inspecionados os settings de Deployment Protection dos projetos Vercel e o painel Cloudflare; ambos os projetos Vercel tinham `Require Log In` habilitado e Zero Trust Free acabara de ser ativado. Naquele snapshot, ainda não existiam projetos Pages e não havia sido criado token/secret Pages. Esse estado foi superado pelos registros posteriores de criação de `vela-internal`/`vela-portal`, configuração Access, geração/salvamento do token e variável `CLOUDFLARE_ACCOUNT_ID`; não descreve o estado atual.
...
- **Snapshot do gate Pages — token CI antes da geração (2026-09-23):** a configuração do token foi preenchida e revisada no painel, com escopo de edição de Pages, expiração de um ano e sem escopos DNS/R2/Workers/Access/billing; naquele momento ainda faltava gerar e salvar `CLOUDFLARE_PAGES_API_TOKEN`. Esse estado foi superado: registros posteriores confirmam que o secret foi salvo no GitHub e `CLOUDFLARE_ACCOUNT_ID` foi criada como Actions Variable. O valor do token não foi lido. A automação continua sem execução até a PR/workflow ser publicada; o estado atual de Access está registrado acima e na seção Fase 0.5.
...
- **Bootstrap autorizado da #739 e revisão de segurança (snapshot anterior, 2026-09-23):** após a validação anterior, a branch Supabase `3c2eefaa-ae1d-40b9-9f66-e8df165bfd47` foi removida conforme autorização do owner; o check `Supabase Preview` havia passado no SHA `d4aaa67e408a23c37cdfe7ad039fd306f9414e70`, e o ambiente estava sem dados. O owner autorizou enviar uma correção de segurança e seguir o bootstrap com Preview temporário novamente, removido ao final, sem DNS ou produção. A revisão encontrou `statuses: write` concedido globalmente, inclusive ao job que executa código da PR; a correção restringiu essa permissão ao job confiável `publish`, limitou os demais jobs e desativou persistência da credencial do checkout não confiável. Também restringiu o token Cloudflare do cleanup ao passo que chama a API. Naquele snapshot, os testes locais tinham passado e o push ainda estava pendente. A consulta do GitHub encontrou apenas o owner como colaborador/autor. A revisão formal permanece exigida pelo ruleset; os registros abaixo refletem o estado posterior. Nenhuma mudança DNS/produção foi autorizada.

- **Revisão independente e sequência de bootstrap (snapshot anterior, 2026-09-23):** a revisão confirmou a falta de bootstrap antes do primeiro deploy e identificou `SUPABASE_ACCESS_TOKEN` disponível ao job inteiro `prepare`. O segredo foi movido para os dois passos que executam a CLI; o teste estático agora cobre esse escopo. No SHA `7a3e8e812604db05d16df3e52594bee7467d9f33`, todos os checks GitHub/Supabase/Vercel passaram e o Preview temporário foi removido depois. O bootstrap, a correção de escopo e os registros atualizados estão nos itens seguintes. A `main` exige uma aprovação formal; só o autor consta como colaborador disponível, então o merge espera reviewer com permissão. Nenhum deploy Pages/DNS/produção ocorreu.
- **Validação final de workflows e bootstrap (2026-09-23):** no head `14532b24546b7b58904977ac345a88233085cd17`, todos os checks passaram, incluindo Supabase Preview e os dois Vercel previews. A branch Supabase `852c359e-cf2d-48e8-8246-929823ec4ca7`, sem dados, associada ao ref `codex/cloudflare-pages-migration`, está ativa e saudável enquanto se conclui esta validação; o owner autorizou removê-la imediatamente ao terminar. Testes locais: 13 Pages, 5 provisionamento, documentação, ESLint e diff check aprovados. Nenhum deployment Pages ou DNS foi alterado. A PR continua draft e o ruleset exige um review aprovado; o único colaborador listado é o autor.
...
28 ignorados; 3.512 testes aprovados e 135 ignorados. O CI GitHub da #739 passou,
mas Supabase Preview foi cancelado por limite de branches concorrentes. Runtime
com artefato e login autorizado/negado em Preview implantado ainda não foi
...
- A configuração do IdP e da política Access foi concluída manualmente pelo owner nos dois projetos. Para o bootstrap, após conferir que os dois Access apps cobrem os hosts de Preview e incluem apenas a identidade autorizada, definir `CLOUDFLARE_PAGES_ACCESS_CONFIGURED=true` antes de publicar a PR de validação; essa variável atesta configuração, não prova runtime. Depois da publicação controlada, testar sessão autorizada e não autorizada nos dois previews. Se qualquer teste falhar, remover o gate e pausar novas publicações até corrigir Access. A política protege deployment previews; o hostname principal `*.pages.dev` e domínios customizados exigem proteção própria, conforme [Customize preview deployments access](https://developers.cloudflare.com/pages/configuration/preview-deployments/) e [Enable Access on `*.pages.dev`](https://developers.cloudflare.com/pages/platform/known-issues/#enable-access-on-pagesdev-domain). Não publicar antes de conferir a configuração no painel.
