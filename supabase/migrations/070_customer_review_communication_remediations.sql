...
ALTER FUNCTION public.save_bl_review(text, timestamptz, jsonb, jsonb, uuid)
  RENAME TO save_bl_review_legacy_070;
REVOKE ALL ON FUNCTION public.save_bl_review_legacy_070(text, timestamptz, jsonb, jsonb, uuid) FROM PUBLIC, anon, authenticated;
...
ALTER FUNCTION public.relink_bl_customer(text, bigint, uuid, text)
  RENAME TO relink_bl_customer_legacy_070;
...
BEGIN
  ...
  RETURN public.relink_bl_customer_legacy_070(p_bl_id, p_customer_id, p_changed_by, p_reason);
END;
$$;
...
ALTER FUNCTION public.import_bl_freight_transactional(jsonb, uuid)
  RENAME TO import_bl_freight_transactional_legacy_070;
REVOKE ALL ON FUNCTION public.import_bl_freight_transactional_legacy_070(jsonb, uuid) FROM PUBLIC, anon, authenticated;
...
BEGIN
  ...
  LOOP
    ...
    END IF;
  END LOOP;
  RETURN public.import_bl_freight_transactional_legacy_070(p_bls, p_changed_by);
END;
$$;
...
ALTER FUNCTION public.register_ledger_invoice_payment(bigint, numeric, text, timestamptz, text, text, text, uuid)
  RENAME TO register_ledger_invoice_payment_legacy_070;
REVOKE ALL ON FUNCTION public.register_ledger_invoice_payment_legacy_070(bigint, numeric, text, timestamptz, text, text, text, uuid) FROM PUBLIC, anon, authenticated;
...
BEGIN
  ...
  LOOP
    PERFORM pg_advisory_xact_lock(hashtextextended('bl:' || v_bl_id, 0));
  END LOOP;
  RETURN public.register_ledger_invoice_payment_legacy_070(p_invoice_id, p_amount_brl, p_method, p_paid_at, p_pix_txid, p_source, p_notes, p_actor);
END;
$$;
...
ALTER FUNCTION public.create_customer_communication_atomic(bigint, text, text, bigint, text, uuid, bigint, integer, uuid, text, text, text, uuid, text[])
  RENAME TO create_customer_communication_atomic_legacy_070;
REVOKE ALL ON FUNCTION public.create_customer_communication_atomic_legacy_070(bigint, text, text, bigint, text, uuid, bigint, integer, uuid, text, text, text, uuid, text[]) FROM PUBLIC, anon, authenticated;
...
BEGIN
  ...
  END IF;
  RETURN public.create_customer_communication_atomic_legacy_070(
